...
- [ ] Move the OpenCode Web Basic-Auth secret from the current ignored host `0600` file into an optional platform secret backend (macOS Keychain first, or a small external secret broker) while preserving non-interactive container startup. OpenCode currently requires the plaintext password at process launch, so the secure-store integration should materialize it only into container tmpfs/process environment and must not claim password-hash support that the upstream server does not provide.
- [ ] Define an explicit non-loopback OpenCode Web exposure mode behind TLS plus stronger upstream authentication/reverse proxy; keep the stock Docker publication on `127.0.0.1` with mDNS disabled and never equate OpenCode Basic Auth with an internet-facing Awoki IAM boundary.
- [ ] Validate the new tmux mouse-off + `set-clipboard external` defaults across VS Code Remote-SSH, Terminal.app, and iTerm2 on macOS: confirm trackpad scrolling no longer triggers gpakosz copy-mode unexpectedly, OSC 52 copies tmux selections to the host clipboard where supported, `<prefix> m` remains a usable opt-in mouse toggle, and document any terminal-specific OSC 52 setting/capability gaps.
