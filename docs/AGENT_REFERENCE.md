# Awoki Detailed Agent Reference

Detailed procedures retained from the original agent instructions. Load only the
sections relevant to the task, not this entire reference at startup. The compact
...
Use `cross_project_code_search` only with 1–8 exact project IDs and user approval.
`all_indexed=true` is rejected; multiple repositories in one project use
`codebase_search`. Never silently widen repository scope.
