...
> Public overview: [`../README.md`](../README.md). Dense maintainer identity and invariants: [`AWOKI_IDENTITY.md`](AWOKI_IDENTITY.md). Current release identity lives in `pyproject.toml` and `.harness/manifest.json`; architecture changes should be justified by realistic-work evidence from [`USEFULNESS_EVALUATION.md`](USEFULNESS_EVALUATION.md).
