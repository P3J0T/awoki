# Awoki — detailed project guide

This guide preserves the full overview, design rationale and workflow examples from the expanded README. Start with the [short README](../README.md) for installation, backup-first upgrades and everyday commands. Commands below assume the repository root; for a shared custom AI service, prefer the guided configurator over manual JSONC editing.
...
See [`docs/ARCHITECTURE.md`](ARCHITECTURE.md) for the detailed topology and [`docs/AWOKI_IDENTITY.md`](AWOKI_IDENTITY.md) for the dense design/invariant map.
...
After the runtime is up, continue with **Create a project and add a repository** below, then start the review in natural language. The complete installation/troubleshooting guide is [`INSTALL.txt`](../INSTALL.txt); the SSH/tmux runtime details are in [`docs/OPENCODE_SSH.md`](OPENCODE_SSH.md).
...
Python builds use reviewed exact constraints; see [dependency updates and compatibility checks](DEPENDENCY_UPDATES.md). This does not change the OpenCode latest/safe policy.
...
See [`docs/RELIABILITY.md`](RELIABILITY.md), [`docs/CONTINUITY.md`](CONTINUITY.md), and [`docs/FILESYSTEM.md`](FILESYSTEM.md) for the detailed contracts.
...
The evaluation plan and keep/simplify/remove criteria are in [`docs/USEFULNESS_EVALUATION.md`](USEFULNESS_EVALUATION.md).
...
- [`docs/OPENCODE_SSH.md`](OPENCODE_SSH.md) — preferred SSH runtime, tmux/OpenCode workflow, reconnect and persistence boundaries
- [`docs/ARCHITECTURE.md`](ARCHITECTURE.md) — runtime/storage/retrieval architecture
- [`docs/AWOKI_IDENTITY.md`](AWOKI_IDENTITY.md) — dense maintainer/future-context identity and invariants
- [`docs/USEFULNESS_EVALUATION.md`](USEFULNESS_EVALUATION.md) — stabilization and real-work evaluation program
- [`docs/OPERATOR_REFERENCE.md`](OPERATOR_REFERENCE.md) — dense operational reference preserved from the former README
- [`docs/COMMANDS.md`](COMMANDS.md) — OpenCode command surface
- [`docs/CODE_SEARCH.md`](CODE_SEARCH.md) — repository retrieval and evidence semantics
- [`docs/CONTINUITY.md`](CONTINUITY.md) — compaction/work/readiness continuity
- [`docs/RELIABILITY.md`](RELIABILITY.md) — verification and acceptance boundaries
- [`docs/BURP.md`](BURP.md) — Burp integration
- [`docs/BACKUP_RESTORE.md`](BACKUP_RESTORE.md) — backup/restore model
